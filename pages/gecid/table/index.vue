    ...
    <v-main>
      ...
      <v-sheet width="100%" color="#fcfdff" class="d-flex flex-column pa-10" align="center">
        ...
        <v-row class="ma-0 pa-0 mt-15" justify="center" align="center">
          <v-data-table
            :headers="headers_students"
            :items="studentsFormatted"
            :items-per-page="15"
            hide-default-footer
            class="ma-0 pa-0"
            @click:row="goToStudentAppointments"
          >
...
export default {
  ...
  methods:
  {
    goToStudentAppointments (student) {
      this.$router.push({ name: 'student-appointments', params: { studentId: student.id } })
    },
    deleteManager (item) {
